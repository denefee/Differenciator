...
static void Taylor_series_loop(Root *taylor_tree, int n_taylor,
    ...
    for (index = 1; index < n_taylor + 1; index++) {
        ...
        if (!is_equal(value, 0)) {
            if (value < 0)
                fprintf(tex_file, " - \\frac{%lg}{%d}", -value,
                        Factorial(index));
            else
                fprintf(tex_file, " + \\frac{%lg}{%d}", value,
                        Factorial(index));
...
void Taylor_series_calculation(Text *text, Root *tree, Constants *f_constants) {
    ...
    if (sscanf(text->meta_string[taylor_series_str].string_point,
               "Number of taylor series: %d", &n_taylor) != 1) {
    ...
    fprintf(tex_file, " %lg",
            Calculate_func(taylor_tree.first_node, 0, f_constants));
...
void Graph_plotter(Text *text, Root *tree, const char *graph_file_name,
    ...
    if (sscanf(text->meta_string[tangent_point_str].string_point,
               "Tangent point: %lg", &tangent_point) != 1) {
    ...
    double df_x =
        Calculate_func(first_diff_tree.first_node, tangent_point, f_constants);
    ...
    if (sscanf(text->meta_string[graph_range_str].string_point,
               ...
               &y_min, &y_max) != 4) {
        printf("Введите границы построения графика в формате (x_min, x_max), "
               "(y_min, y_max):\n");
    ...
    save_node(graph_file, tree->first_node, nullptr, f_constants);
    if (!isnan(df_x) and !isnan(f_x))
        fprintf(graph_file, ", %lg * (x - %lg) + %lg", df_x, tangent_point,
                f_x);
    fclose(graph_file);


    std::stringstream out;
    out << "gnuplot " << graph_file_name;
...
void Calculate_function_in_point(Text *text, Root *tree,
    ...
    if (sscanf(text->meta_string[func_calc_str].string_point,
               "Function calculation point: %lg", &f_x_value) != 1) {
        printf(
            "Введите точку, в которой требуется вычислить значение функции: ");
        scanf("%lg", &f_x_value);
    }
...
void Calculate_derivative_in_point(Text *text, Root *diff_tree,
    ...
    if (sscanf(text->meta_string[derivative_calc_str].string_point,
               "Derivative calculation point: %lg", &df_x_value) != 1) {
        printf("Введите точку, в которой требуется вычислить значение "
               "производной: ");
        scanf("%lg", &df_x_value);
    }
...
void Taking_nth_derivative(Text *text, Root *tree, Root *diff_tree) {
    ...
    if (sscanf(text->meta_string[derivative_n_str].string_point,
               "Derivative power: %d", &derivative_n) != 1) {
